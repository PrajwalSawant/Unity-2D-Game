...
public class Player : MonoBehaviour, IDamageable, ICollectable{
	...
	void Move(){
		...
		//{
		//	if (GameManager.Instance.BootsOfFlight == true)
		//	{
		...
		if (CrossPlatformInputManager.GetButtonDown("B_Button")|| Input.GetKeyDown("space"))
		{
			//if (GameManager.Instance.BootsOfFlight == true)
			//{
			//	Debug.Log("Extra Jump");
			//	m_rBody2D.velocity = new Vector2(m_rBody2D.velocity.x, m_jumpForce + 4);
			//	m_playerAnim.Jump(true);
			//	StartCoroutine(ResetJumpRoutine());
			//}
			//else
		...
		else if (DoubleJump == true && GameManager.Instance.BootsOfFlight == true)
			{
	...
	public void Damage(){
		...
		//Play Hit Animation ;  m_playerAnim.Hit();  TODO: Play hit sound
